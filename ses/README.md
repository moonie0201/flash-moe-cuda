...
| SSD only (cold, no cache) | 0.77 | Every expert loaded from SSD |
| GPU VRAM cache only | 1.09 | 6.8GB, ~1980 experts |
| **3-tier cache (this work)** | **1.35** | **+17% vs llama.cpp** |

## How It Works

The 397B model has 60 layers × 512 experts. Only 10 experts activate per token — but that's still 600 expert loads per token from a 120GB file on SSD.
